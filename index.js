...
function customAssign(...objects) {
  if (objects.length < 2) {
    console.error(`Помилка: Має бути передано принаймні два об'єкти.`);
    return null;
  } // Перевірка, чи передано достатньо об'єктів якщо ні повертаємо "Помилка: Має бути передано принаймні два об'єкти."
  const result = Object.assign(...objects); // Використання методу `assign` для об'єднання об'єктів
  for (const el in result) {
    if (typeof result[el] === "number") {
      result[el] = result[el].toString();
    }
  ...
  return result; // Повернення нового об'єкта з об'єднаними та зміненими властивостями
}
...
function customEntries(obj) {
  if (typeof obj !== "object") {
    console.error(`Помилка: Аргумент не є об'єктом.`);
    return null;
  } // Перевірка, чи переданий аргумент є об'єктом, якщо ні повертаємо "Помилка: Аргумент не є об'єктом."
  const array = Object.entries(obj); // Використання методу `entries` для отримання масиву з усіма властивостями та значеннями
  return array; // Повернення масиву властивостей та значень
}
...
console.log(customEntries("Hello World!"));
...
function customObjectFromEntries(entries) {
  if (!Array.isArray(entries)) {
    console.error(`Помилка: Вхідний аргумент має бути масивом`);
    return null;
  ...
  return result; // Перевірка, чи ключ  є числом
...
console.log(customObjectFromEntries(3.14, 15, "123"));
...
console.log(checkProperty({ a: 1, b: 2, c: 3 }, 15));
console.log(checkProperty("Hello", 15));
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const newObj = Object.assign(obj1, obj2); // Використовуємо метод Object.assign() для додавання властивостей obj2 до obj1
  Object.freeze(newObj); // Використовуємо метод Object.freeze() для "заморожування" об'єкта, запобігаючи подальшим змінам
...
function filterObjectsByKey(arr, key) {
  ...
  return arr.filter((obj) => obj.hasOwnProperty(key)); // Використовуємо метод filter() для вибірки об'єктів, що містять вказаний ключ
...
function checkOwnership(obj1, obj2, key) {
  ...
  return false; // Використовуємо метод hasOwnProperty() для перевірки наявності ключа в обох об'єктах.
...
console.log(
  checkOwnership(
    { name: "Jane", lastname: "Doe", profession: "Dentist" },
    { a: 1, b: 2, c: 4 },
    "a"
  )
);
console.log(checkOwnership({ a: 1, b: 2, c: 3 }, { a: 1, b: 2, c: 4 }, "d"));
// Завдання: 11
...
function getObjectValuesSum(obj) {
  ...
  const sum = values.reduce((acc, value) => acc + value, 0); // Обчислюємо суму значень
...
function convertArrayToObj(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (Object.hasOwnProperty(key)) {
      console.warn(`У масиві є дубльований ключ: ${key}`);
    }
  ...
  return obj; // Застосовуємо метод Object.fromEntries() для створення об'єкта
}
